 ...
 * sì, la strada di `Manichino3D` (three in-process, dentro Expo Go, zero
 * manutenzione nativa) resta aperta e non serve valutare altri motori.
 *
 ...
 * in fuori lungo la normale (vedi `scripts/genera_capi.py`). Questo rende il
 * weight transfer esatto invece che approssimato — i vertici del capo *sono*
 * vertici del corpo, quindi JOINTS_0 e WEIGHTS_0 si copiano invariati — ed è il
 * motivo per cui la maglietta aderisce senza compenetrare.
 *
 ...
 * dell'armadio, stesa sulla geometria da `scripts/vesti_da_foto.py`. È il primo
 * pezzo di `docs/adr/0004` che funziona — il capo non è più un colore.
 *
 ...
 *
 * In `assets/3d/` i capi stanno in due versioni: quelle a tinta unita sono
 * l'ingresso di `vesti_da_foto.py` e non finiscono nel bundle, quelle `-foto`
 * sono ciò che la schermata carica davvero.
 */
  ...
  return (
    <Toccabile
      ...
      style={{
        ...
        borderWidth: 1,
        borderColor: acceso ? colori.citron : 'rgba(247,244,239,0.14)',
        backgroundColor: acceso ? 'rgba(215,244,92,0.14)' : 'rgba(247,244,239,0.04)',
      }}
    >
      <Etichetta taglia={12} colore={acceso ? colori.citron : 'rgba(247,244,239,0.6)'}>
        {etichetta}
      </Etichetta>
    </Toccabile>
  )
